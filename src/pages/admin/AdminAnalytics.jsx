  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Tổng doanh thu', value: formatPrice(totalRevenue), icon: TrendingUp, color: 'from-orange-500 to-orange-600', bg: 'bg-orange-50', sub: 'tất cả sự kiện' },
          { label: 'Đơn hàng', value: totalOrders, icon: ShoppingBag, color: 'from-purple-500 to-purple-600', bg: 'bg-purple-50', sub: `TB ${formatPrice(avgOrderValue)}` },
          { label: 'Vé đã bán', value: totalTickets, icon: Ticket, color: 'from-blue-500 to-blue-600', bg: 'bg-blue-50', sub: `${totalOrders} đơn hàng` },
          { label: 'Sự kiện có DT', value: eventRows.length, icon: Calendar, color: 'from-emerald-500 to-emerald-600', bg: 'bg-emerald-50', sub: 'đang hoạt động' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            {last7.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full relative flex items-end justify-center" style={{ height: '120px' }}>
                  <div className="w-full bg-gradient-to-t from-orange-500 to-purple-500 rounded-t-lg transition-all duration-700 hover:opacity-80"
          ...
          {last7.every(d => d.revenue === 0) && (
            <p className="text-xs text-gray-400 text-center mt-4">Chưa có dữ liệu 7 ngày qua</p>
          )}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <h3 className="text-sm font-bold text-gray-900 mb-5 flex items-center gap-2">
            <ArrowUp className="w-4 h-4 text-emerald-500" /> Top sự kiện theo doanh thu
          </h3>
          {eventRows.length === 0 ? (
              ...
              {eventRows.slice(0, 5).map((e, i) => (
                <div key={e.name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                        ...
                        i === 2 ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'
                      }`}>{i + 1}</span>
                      <p className="text-xs text-gray-700 font-medium truncate max-w-[140px]">{e.name}</p>
                    </div>
                  ...
                  <div className="flex gap-4 mt-1">
                    ...
                    <span className="text-xs text-emerald-600 font-medium">{e.checkedIn} check-in</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="text-sm font-bold text-gray-900">Chi tiết theo sự kiện</h3>
        </div>
        {eventRows.length === 0 ? (
                ...
                {eventRows.map((e, i) => {
                  ...
                  return (
                    <tr key={e.name} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs font-semibold text-gray-800 max-w-[200px] truncate">{e.name}</td>
                      <td className="px-4 py-3 text-xs font-bold text-orange-600 whitespace-nowrap">{formatPrice(e.revenue)}</td>
                      <td className="px-4 py-3 text-xs text-gray-600">{e.orders}</td>
                      <td className="px-4 py-3 text-xs text-gray-600">{e.tickets}</td>
                      <td className="px-4 py-3 text-xs text-emerald-600 font-semibold">{e.checkedIn}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full" style={{ width: `${checkInRate}%` }} />
                          </div>
                          <span className="text-xs text-gray-500 font-medium">{checkInRate}%</span>
                        </div>
                      </td>
                    </tr>
                  );
