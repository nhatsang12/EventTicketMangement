          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <div onClick={() => fileRef.current?.click()}
                className="relative h-36 border-2 border-dashed border-gray-200 hover:border-orange-400 rounded-xl overflow-hidden cursor-pointer transition-all group bg-gray-50">
                {form.imagePreview ? (
              ...
              <input type="url" placeholder="https://..." value={form.imageUrl}
                ...
                className="w-full mt-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-700 placeholder-gray-400 focus:outline-none focus:border-orange-400 focus:bg-white" />
            </div>

            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Tên sự kiện *</label>
              <input required type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder="Nhập tên sự kiện..." className={inputCls} />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Ngày *</label>
              <input required type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className={inputCls} />
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Giờ</label>
              <input type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))} className={inputCls} />
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Địa điểm *</label>
          ...
          {filtered.map((event, i) => (
            <div key={event.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:border-orange-300 hover:shadow-md transition-all group">
              <div className="relative h-36 bg-gray-100">
                ...
                {event.category && (
                  <span className="absolute top-3 left-3 px-2 py-0.5 bg-white/90 backdrop-blur-sm text-gray-700 text-xs rounded-full font-semibold border border-gray-200">
                    {event.category}
                  </span>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-xs text-gray-500 mb-5">
              Bạn có chắc muốn xóa sự kiện <span className="text-gray-900 font-semibold">"{events[deleteConfirm]?.name}"</span>? Hành động này không thể hoàn tác.
            </p>
